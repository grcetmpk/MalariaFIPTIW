...
numevents 
dim(data_children) #8924 x 27
length(unique(data_children$Participant_Id)) #252 children
table(data_children$Observation_type_BFO_0000015) # 252 enrollment, 5185 sched, 3487 unsched
...
dim(data_children_malaria) # 1531/9896 (~15.5%) of observations have malaria diagnosis at some point
length(unique(data_children_malaria$Participant_Id)) # 177/252 (~70%) of patients had malaria at some point
...
#### only 2 observations had antimalarials within one week of observation. 
sum(trtwithinoneweek_full)


# create a variable for protected water sources
data_children$waternotpiped <- ifelse(data_children$Drinking_water_source_ENVO_00003064 %in% c("Public tap","River/stream",
                        "Protected public well", "Protected spring" ,
                        "Borehole", "Open public well", "Pond/lake", "Unprotected spring"), 1, 0)


data_children_baseline <- data_children[data_children$Observation_type_BFO_0000015 == "Enrollment", ]
table(data_children_baseline$waternotpiped) #236/252 children have unpiped water sources
...
exposuremod <- glm(factor(waternotpiped) ~ `Age_(years)_OBI_0001169` + `Sub-county_in_Uganda_EUPATH_0000054` +
...
summary(exposuremod)
#  wealth index and number of persons living in house significant

outcomemod <- glm(factor(Malaria) ~ `Age_(years)_OBI_0001169` + `Sub-county_in_Uganda_EUPATH_0000054` +
                    `Household_wealth_index,_categorical_EUPATH_0000143` + `Food_problems_per_week_EUPATH_0000029` +
                    waternotpiped + Human_waste_facilities_EUPATH_0000335 +
                    Observation_type_BFO_0000015 + Dwelling_type_ENVO_01000744 + Persons_living_in_house_count_EUPATH_0000019,
                  family = binomial(link = "logit"), data = data_children)
summary(outcomemod)
# age, subcounty, wealth index, food problems,  waste faciltiies, observation type, persons living in house  significant


#include covariates in model that are confounders and predictive only of outcome 
# removed subcounty, food problems, human waste as had high standard errors and produce NAs for PS
psmod <- glm(factor(waternotpiped) ~ `Age_(years)_OBI_0001169` + #`Sub-county_in_Uganda_EUPATH_0000054` +
               `Household_wealth_index,_categorical_EUPATH_0000143` +  #`Food_problems_per_week_EUPATH_0000029`+
               #Human_waste_facilities_EUPATH_0000335 +
...
iptw <- 1/ps*as.numeric(data_children$waternotpiped)+1/(1-ps)*(1-as.numeric(data_children$waternotpiped))
summary(iptw)
sum(iptw > 5)/length(iptw)
...
# weight stabilizer
delta.hat <- coxph(Surv(Time_lag, `Time_since_enrollment_(days)_EUPATH_0000191`, Observed) ~ factor(waternotpiped) - 1, data = data_children)$coef

# model with aux variables
intensitymod <- coxph(Surv(Time_lag, `Time_since_enrollment_(days)_EUPATH_0000191`, Observed) ~ factor(waternotpiped) + `Age_(years)_OBI_0001169` +
...
iiw <- exp(cbind(data_children$waternotpiped)%*%delta.hat)/exp(cbind(data_children$waternotpiped, 
...
hist(fiptiw)
summary(fiptiw)
sum(fiptiw > 5)/length(fiptiw)

threshold <- quantile(fiptiw, 0.95) #4.85\
threshold
...
finalmod_unadj <- glm(factor(Malaria) ~ factor(waternotpiped) + bs(data_children$`Time_since_enrollment_(days)_EUPATH_0000191`, 
...
finalmod_iptw <- glm(factor(Malaria) ~ factor(waternotpiped) + bs(data_children$`Time_since_enrollment_(days)_EUPATH_0000191`, 
...
finalmod_iiw <- glm(factor(Malaria) ~ factor(waternotpiped) + bs(data_children$`Time_since_enrollment_(days)_EUPATH_0000191`, 
                                                               degree=3,knots=c(terti)), data=data_children, 
...
finalmod_fiptiw <- glm(factor(Malaria) ~ factor(waternotpiped) + 
...
finalmod_fiptiw_trimmed <- glm(factor(Malaria) ~ factor(waternotpiped) + bs(data_children$`Time_since_enrollment_(days)_EUPATH_0000191`, 
                                                                          degree=3,knots=c(terti)), data=data_children, family=quasibinomial(link="logit"), 
...
table_demog <- rbind(sum_tab("Age at Enrollment", data_children_baseline$`Age_at_enrollment_(years)_EUPATH_0000120`, "continuous"),
                     ...
                     sum_tab("Unpiped Water Source", data_children$waternotpiped, 'categorical'),
...
kable(table_demog, format = "latex", booktabs = T) %>%
  ...
  pack_rows("Drinking Water Source", 10, 19) %>%
  pack_rows("Unpiped Water Source", 20, 21) %>%
  pack_rows("Dwelling Type", 22, 23) %>%
  pack_rows("Food Problems per Week", 24, 28) %>%
  pack_rows("Waste Facilities", 29, 36) 
