...
prop_noninformative <- length(which(data_children_lastobs$Reason_for_withdrawal_EUPATH_0000208 %in% 
                                      c("Completed study", "Moved out of area")))/dim(data_children_lastobs)[1] 
1 - prop_noninformative #69.34\% of individuals were censored for non-negligible reasons.
...
dim(data_children) #9525 observations
...
potentialtimes <- c(seq(0, 1781, by = 1)) #study end at day 1781
...
summary(iptw) #max 11.35
sum(iptw > 5)/length(iptw) #4.67%
sum(iptw > 10)/length(iptw) #0.21%
...
summary(iiw) #max 11.16
sum(iiw>5)/length(iiw) #2.78\%
sum(iiw>10)/length(iiw) #0.01%
...
hist(fiptiw)
summary(fiptiw) #max 36.22
sum(fiptiw > 5)/length(fiptiw) #24.30%
sum(fiptiw > 10)/length(fiptiw) #6.64%
sum(fiptiw > 20)/length(fiptiw) #1.37%
