...
summary(outcomemod)
#   wealth index,  food problems per week, waste faciltiies, observation type, num in house highly significant
...
hist(fiptiw)
summary(fiptiw) #max 65.0
sum(fiptiw > 5)/length(fiptiw) #2.78%
sum(fiptiw > 10)/length(fiptiw) #1.07%
sum(fiptiw > 20)/length(fiptiw) #0.02%
