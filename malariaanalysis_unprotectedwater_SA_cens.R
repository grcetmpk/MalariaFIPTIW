## same analysis but include censored individuals
...
length(unique(data_children$Household_Id.x)) # we have 287 unique households
...
numevents # min 1 mean 30.2 max 95.0
dim(data_children) #8681 x 25
length(unique(data_children$Participant_Id)) #287 children
table(data_children$Observation_type_BFO_0000015) # 287 enrollment, 5163 sched, 3231 unsched
...
dim(data_children_malaria) # 1322/8681 (~15.2%) of observations have malaria diagnosis at some point
length(unique(data_children_malaria$Participant_Id)) # 218/287 (~76%) of patients had malaria at some point
...
table(data_children_baseline$waternotprotected) #69/287 have unprotected water sources
#no missingness :D
...
summary(outcomemod)
#  subcounty, wealth index,  observation type highly significant


#include covariates in model that are confounders and predictive only of outcome 
psmod <- glm(factor(waternotprotected) ~ #`Sub-county_in_Uganda_EUPATH_0000054` + #`Age_(years)_OBI_0001169` + 
               `Household_wealth_index,_categorical_EUPATH_0000143` +  #`Food_problems_per_week_EUPATH_0000029`+
               #Human_waste_facilities_EUPATH_0000335 + 
               #Persons_living_in_house_count_EUPATH_0000019 + 
               Observation_type_BFO_0000015,
             family = binomial(link = "logit"), data = data_children)
summary(psmod) # subcounty inflated variance
...
summary(iptw) #mac 5.40
sum(iptw > 5)/length(iptw) #0.02%
...
summary(iiw) #max 1.821
sum(iiw>5)/length(iiw) #0

fiptiw <- iiw*iptw
data_children$fiptiw <- fiptiw

hist(fiptiw)
summary(fiptiw) #max 8.42
sum(fiptiw > 5)/length(fiptiw) #7.6%
sum(fiptiw > 10)/length(fiptiw) #7.6
sum(fiptiw > 20)/length(fiptiw) #7.6%
...
kable(table_demog, format = "latex", booktabs = T) %>%
  ...
  pack_rows("Waste Facilities", 27, 35) %>%
  pack_rows("Reason for Withdrawal", 37, 43)
