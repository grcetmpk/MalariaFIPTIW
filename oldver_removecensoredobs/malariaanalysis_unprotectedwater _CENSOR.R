...
data_children[which(data_children$Human_waste_facilities_EUPATH_0000335 == "[\"Uses neighbours\",\"No facility/bush/field\"]"), 23] <- "[\"No facility/bush/field\"]" 
...
dim(data_children) #4104 x 25
length(unique(data_children$Participant_Id)) #133 
table(data_children$Observation_type_BFO_0000015) # 133 enrollment, 2195 sched, 1776 unsched
...
dim(data_children_malaria) # 773/4104 (~18.8%) of observations have malaria diagnosis at some point
length(unique(data_children_malaria$Participant_Id)) # 90/133 (~67.7%) of patients had malaria at some point
...
table(data_children_baseline$waternotprotected) #28/133 unprotected water 
#no missingness :D
...
potentialtimes <- c(seq(0, 1775, by = 1))
...
fullpotentialdata$HWF_UPLNS <- ifelse(fullpotentialdata$Human_waste_facilities_EUPATH_0000335 == "[\"Uncovered pit latrine no slab\"]" , 1, 0)
...
#were calrulated when making the counterfactual data
...
summary(iptw) #max 11.99
sum(iptw > 5)/length(iptw) #3.56%
sum(iptw > 10)/length(iptw) #0.15%
...
Z <- cbind(observed_potentialdata$waternotprotected, observed_potentialdata$currentage,
           ...
           observed_potentialdata$HWF_VIP, observed_potentialdata$Dwellingtypenum,
...
summary(iiw) #max 20.30
sum(iiw>5)/length(iiw) #5.65%
sum(iiw>10)/length(iiw) #0.71%
sum(iiw > 20)/length(iiw) #0.02\%
...
hist(fiptiw)
summary(fiptiw) #max 46.601
sum(fiptiw > 5)/length(fiptiw) #43.64%
sum(fiptiw > 10)/length(fiptiw) #9.30%
sum(fiptiw > 20)/length(fiptiw) #3.35%
